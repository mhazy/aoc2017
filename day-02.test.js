...
const testDivisibleInput = `5 9 2 8
9 4 7 3
3 8 6 5`;

const puzzleInput = `121	59	141	21	120	67	58	49	22	46	56	112	53	111	104	130
...
describe("Day 2 - Corruption Checksum", () => {
  it("convert input to array of array of numbers", () => {
    const result = dayTwo.parseInput(testInput);
  ...
  it("calculates spreadsheet checksum", () => {
    expect(dayTwo.calculateSheetChecksum(testInput)).toBe(18);
  });

  it("calulates the puzzle checksum, part one", () => {
    expect(dayTwo.calculateSheetChecksum(puzzleInput)).toBe(32121);
  });

  it("returns true if number is integer, false if not", () => {
    expect(dayTwo.isInteger(2)).toBe(true);
    expect(dayTwo.isInteger(2.5)).toBe(false);
  });

  it("returns divisible numbers in row", () => {
    expect(dayTwo.findDivisibles([5, 9, 2, 8])).toMatchObject([8, 2]);
  });

  it("calculates the divisible checksum", () => {
    expect(dayTwo.calculateDivisibleRowChecksum([5, 9, 2, 8])).toBe(4);
  });

  it("calculates spreadsheet divisible checksum", () => {
    expect(dayTwo.calculateDivisibleSheetChecksum(testDivisibleInput)).toBe(9);
  });

  it("calulates the puzzle checksum, part two", () => {
    expect(dayTwo.calculateDivisibleSheetChecksum(puzzleInput)).toBe(197);
  });
});
